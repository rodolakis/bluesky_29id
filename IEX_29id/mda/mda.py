...
class scanDim:
    ...
    def __init__(self):
        ...
        # [1..n]  1 means this is the "innermost" or only scan dimension
        self.rank = 0
...
def readScan(scanFile, verbose=0, out=sys.stdout, unpacker=None):
    ...
    for j in range(scan.np):
        ...
        if length:
            scan.p[j].name = u.unpack_string().decode('utf-8')
        ...
        if length:
            scan.p[j].desc = u.unpack_string().decode('utf-8')
        ...
        if length:
            scan.p[j].step_mode = u.unpack_string().decode('utf-8')
        ...
        if length:
            scan.p[j].unit = u.unpack_string().decode('utf-8')
        ...
        if length:
            scan.p[j].readback_name = u.unpack_string().decode('utf-8')
        ...
        if length:
            scan.p[j].readback_desc = u.unpack_string().decode('utf-8')
        ...
        if length:
            scan.p[j].readback_unit = u.unpack_string().decode('utf-8')
    ...
    for j in range(scan.nd):
        ...
        if length:
            scan.d[j].name = u.unpack_string().decode('utf-8')
        ...
        if length:
            scan.d[j].desc = u.unpack_string().decode('utf-8')
        ...
        if length:
            scan.d[j].unit = u.unpack_string().decode('utf-8')
    ...
    for j in range(scan.nt):
        ...
        if length:
            scan.t[j].name = u.unpack_string().decode('utf-8')
...
def readMDA(fname=None, maxdim=4, verbose=0, showHelp=0, outFile=None, useNumpy=None, readQuick=False):
    ...
    if ((rank > 1) and (maxdim > 1)):
        ...
        for i in range(dim[0].curr_pt):
            ...
            if (dim[0].plower_scans[i] == 0):
                ...
                if verbose:
                    out.write("1D point %d/%d; seek = 0x%x\n" %
                              (i, dim[0].curr_pt, dim[0].plower_scans[i]))

                if (i == 0):

                    (s, detToDat) = readScan(scanFile,
                                             max(0, verbose-1), out, unpacker=u)
                    ...
                    if readQuick:
                        ...
                        s = readScanQuick(scanFile, unpacker=u,
                                          detToDat_offset=detToDat)

                    else:

                        (s, junk) = readScan(scanFile, max(
                            0, verbose-1), out, unpacker=u)
            ...
            if (dim[0].plower_scans[i] == 0):
                ...
                for j in range(s1.curr_pt):
                    ...
                    for k in range(numD):
                        ...
                        dim[2].d[k].data[i].append(
                            [0]*len(dim[2].d[k].data[0]))

            else:

                scanFile.seek(dim[0].plower_scans[i])

                (s1, detToDat) = readScan(scanFile,
                                          max(0, verbose-1), out, unpacker=u)
                ...
                for j in range(s1.curr_pt):
                    ...
                    if (s1.plower_scans[j] == 0):
                        ...
                        if verbose:
                            out.write("2D point [%d,%d]/[%d,%d]; declining to seek null file loc; appending zeros\n" %
                        ...
                        if verbose:
                            out.write("2D point %d/%d; seek = 0x%x\n" %
            ...
            if (dim[0].plower_scans[i] == 0):
                ...
                for j in range(s1.curr_pt):
                    ...
                    if (s1.plower_scans[j] == 0):
                        ...
                        if verbose:
                            out.write("2D point [%d,%d]/[%d,%d]; declining to seek null file loc\n" %
                        ...
                        for k in range(s2.curr_pt):
                            ...
                            if (s2.plower_scans[k] == 0):
                                ...
                                if verbose:
                                    out.write("4D point [%d,%d,%d]/[%d,%d,%d]; seek = 0x%x\n" %
    ...
    dict['sampleEntry'] = ("description", "unit string",
                           "value", "EPICS_type", "count")
    ...
    dict['ourKeys'] = ['sampleEntry', 'filename', 'version', 'scan_number',
                       'rank', 'dimensions', 'acquired_dimensions', 'isRegular', 'ourKeys']
    ...
    if pExtra:
        ...
        for i in range(numExtra):
            ...
            if n:
                desc = u.unpack_string().decode('utf-8')
            ...
            if EPICS_type != 0:   # not DBR_STRING; array is permitted
                ...
                if n:
                    unit = u.unpack_string().decode('utf-8')
            ...
            if EPICS_type == 0:  # DBR_STRING
                ...
                if n:
                    value = u.unpack_string().decode('utf-8')
    ...
    if verbose or showHelp:
        ...
        out.write(
            "dim[0] = dictionary of %d scan-environment PVs\n" % (len(dim[0])))
    ...
    if showHelp:
        ...
        print("      time      - date & time at which scan was started: %s" %
              (dim[1].time))

        print("      name - name of scan record that acquired this dimension: '%s'" %
              (dim[1].name))

        print("      curr_pt   - number of data points actually acquired: %d" %
              (dim[1].curr_pt))

        print("      npts      - number of data points requested: %d" %
              (dim[1].npts))

        print(
            "      nd        - number of detectors for this scan dimension: %d" % (dim[1].nd))

        print("      d[]       - list of detector-data structures")

        print(
            "      np        - number of positioners for this scan dimension: %d" % (dim[1].np))

        print("      p[]       - list of positioner-data structures")

        print(
            "      nt        - number of detector triggers for this scan dimension: %d" % (dim[1].nt))
...
def skimScan(dataFile):
    ...
    if (scan.rank > 20) or (scan.rank < 0):
        ...
        print("* * * skimScan('%s'): rank > 20.  probably a corrupt file" %
              dataFile.name)
            ...
            if (rank > 2):
                ...
                if (rank > 3):
                    ...
                    for k in range(m.scan.inner[i].inner[j].npts):
                        ...
                        m.scan.inner[i].inner[j].pLowerScans.append(
                            thisScan.offset)
            ...
            for i in range(m.scan.npts):
                ...
                if (rank > 3):  # 4D scan
                    ...
                    for j in range(m.scan.inner[i].npts):
                        ...
                        p.pack_farray(
                            m.scan.inner[i].inner[j].npts, m.scan.inner[i].inner[j].pLowerScans, p.pack_int)
        ...
        for j in range(s[1].npts):
            ...
            for k in range(s[2].npts):
                ...
                for l in range(s[3].npts):
                    ...
                    s[3].d[i].data[j][k][l] = op(
                        s[3].d[i].data[j][k][l], scalar)
