...
from ScanFunctions_IEX import getE
from ScanFunctions_IEX import ScalerInt
from ScanFunctions_IEX import Scan_FillIn_Table
from ScanFunctions_IEX import Scan_Go
from ScanFunctions_IEX import Clear_Scan_Positioners
#JWF 07/29/21 added load of mvth,mv... to see if it fixes the error
from ScanFunctions_IEX import mvth
from ScanFunctions_IEX import mvchi
from ScanFunctions_IEX import mvphi
from ScanFunctions_IEX import mvtth
...
# Define real motor:
#MOTOR_PV_OMEGA = "29idKappa:m8"
#MOTOR_PV_CHI = "29idKappa:m7"
#MOTOR_PV_PHI = "29idKappa:m1"
#MOTOR_PV_TTH = "29idKappa:m9"
...
def cahkl(h,k,l,solution=0,q=None):
    ...
    try:
        ...
        if q == None:
            ...
            print('Return = {:s}'.format('e_theta,e_chi,e_phi,e_tth,k_th,k_kap,k_phi'))
...
def cahkl_table(reflections):
    ...
    try:
        ...
        print('LAMBDA = {:.3f} \thv = {:.3f}'.format(energy_Ang,energy_eV))
        print('Mode = {:s}'.format(fourc.calc.engine.mode))
        print(fourc.forward_solutions_table(reflections, full=True))
    except:
...
def setor(h,k,l,*args):
    """Define a reflection in fourc for h,k,l,omega"""
    if len(args)==4: tth,omega,chi,phi = args
    elif 0<len(args)<4: 
        print('Please specify 4 angles: tth,omega,chi,phi')
        return
    else:
        tth=tth_rbv.get()
        omega=omega_rbv.get()
    ...
    position=fourc.calc.Position(
        tth=tth, omega=omega, chi=chi, phi=phi))
    print(f"tth={tth:.3f}, omega={omega:.3f}, chi={chi:.3f}, phi={phi:.3f}")
    return(r1)
...
def UBcalc(r1,r2,sync=True):
    ...
    # Updating PV UBmatrix
    caput('29idKappa:UBmatrix',UB.reshape(9,1))   # flattened to a 9x1 matrix
    # Updating PV UBor1
    h=fourc.reflections_details.get()[0]['reflection']['h']
    k=fourc.reflections_details.get()[0]['reflection']['k']
    l=fourc.reflections_details.get()[0]['reflection']['l']
    omega=fourc.reflections_details.get()[0]['position']['omega']
    chi=fourc.reflections_details.get()[0]['position']['chi']
    phi=fourc.reflections_details.get()[0]['position']['phi']
    tth=fourc.reflections_details.get()[0]['position']['tth']
    caput('29idKappa:UBor1',np.array([h,h,l,omega,chi,phi,tth]))
    # Updating PV UBor2
    h=fourc.reflections_details.get()[1]['reflection']['h']
    k=fourc.reflections_details.get()[1]['reflection']['k']
    l=fourc.reflections_details.get()[1]['reflection']['l']
    omega=fourc.reflections_details.get()[1]['position']['omega']
    chi=fourc.reflections_details.get()[1]['position']['chi']
    phi=fourc.reflections_details.get()[1]['position']['phi']
    tth=fourc.reflections_details.get()[1]['position']['tth']
    caput('29idKappa:UBor2',np.array([h,h,l,omega,chi,phi,tth]))
