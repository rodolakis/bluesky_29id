...
Created on Fri Feb 14 09:21:40 2020

@author: 29iduser

 To convert tabs to space or space to tabs:
 IMPORTANT - does need a new destination 
     file.py > file_new.py 
 NOT file.py > file.py (deletes all the content!!!)
...
## WARNING: beware delimiters in files loading 
...
def Folder_Kappa(UserName,**kwargs):
    """
    Create and sets (if create_only=False) all the folders the current run and ARPES user 
        Mode = "User" or "He"; automatically set to "Staff" if UserName is "Staff" 
    Sets the FileName for logging to be "UserName/YYYYMMDD_log.txt using logname_Set()"
    ...
    **kwargs:
    ...
    kwargs.setdefault('create_only',False)
    kwargs.setdefault('run',Check_run())
    kwargs.setdefault('scanIOC','Kappa')
    kwargs.setdefault('Mode','User')
    kwargs.setdefault('ftp',False)
    ...
    if kwargs['create_only'] == False:
        ...
        Folder_mda(run,folder,UserName,scanIOC)
        # Set up SPEC folder:
        Folder_SPEC(run,folder,UserName)
        # Set up MPA folder:
        #Folder_MPA(run,folder,UserName)
        print('06/30/2021: cammented our Folder_MPA(...)')
        print("\nScanIOC set to:", scanIOC)
        sleep(5)
        SaveStatus=caget('29id'+scanIOC+':saveData_status',as_string=True)
        SaveMessage=caget('29id'+scanIOC+':saveData_message',as_string=True)
        print("\nSave Status "+scanIOC+": "+SaveStatus+" - "+SaveMessage)
        logname_set(scanIOC=scanIOC)
    print("\nREMINDER: Reset_Scan('"+scanIOC+"'); Reset_CA_all(); Reset_Mono_Limits(); Sync_SmarAct_Motor(); Sync_PI_Motor(); Sync_Euler_Motor()")
...
def logprint(comment,**kwargs):
    ...
    try:
        ...
        if not isfile(join(FilePath, FileNameWithSubFolder)):
            SaveFile_Header(FilePath,FileNameWithSubFolder)
        with open(join(FilePath, FileNameWithSubFolder),'a') as myfile:
...
##### When adding/removing detector to the trigger:
#   CA_Passive()
#   CA_Average()
#   CA_Name()
#   CA_Live_StrSeq()
#   Detector_Triggers_StrSeq() x2
#   BeforeScan_StrSeq(ioc)
